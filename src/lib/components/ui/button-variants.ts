export type ButtonVariant = 'primary' | 'primaryBlock' | 'toolbar' | 'toolbarSecondary' | 'ghost' | 'link';

/** Shared class map — use `toolbar` / `toolbarSecondary` for page header CTAs (and matching `<a>` links). */
export const buttonVariants: Record<ButtonVariant, string> = {
	...
	toolbarSecondary:
		'inline-flex h-11 min-h-[var(--touch-target)] shrink-0 items-center justify-center gap-1.5 whitespace-nowrap rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] px-3.5 text-sm font-semibold leading-none text-[var(--color-text)] transition-colors duration-200 hover:bg-[var(--color-surface-hover)] focus:outline-none focus:ring-2 focus:ring-[var(--color-ring)] focus:ring-offset-2 focus:ring-offset-[var(--color-bg)] disabled:cursor-not-allowed disabled:opacity-70',
	ghost:
