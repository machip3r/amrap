...
export async function saveGymThemeAction(
...
export async function uploadGymLogoAction(
  ...
  const { error } = await supabase.rpc("update_gym_branding", {
    p_gym_id: workspace.gymId,
    ...(mode === "light"
      ? { p_logo_url_light: path }
      : { p_logo_url_dark: path }),
  });
  ...
  const storedPath =
    mode === "light" ? gymRow?.logo_url_light : gymRow?.logo_url_dark;
  if (!storedPath) {
    console.error("uploadGymLogoAction missing path after rpc", mode);
    return { error: d.settings.logoError };
  }

  const logoUrl = gymLogoPublicUrl(
    storedPath,
    gymRow?.updated_at ?? Date.now(),
  );
...
export async function removeGymLogoAction(
  _prev: SettingsActionState,
  formData: FormData,
): Promise<SettingsActionState> {
  const locale = localeFromForm(formData);
  ...
  await Promise.all(
    ["png", "jpg", "jpeg", "webp", "svg"].map((ext) =>
      supabase.storage
        .from("gym-logos")
        .remove([`${workspace.gymId}/logo-${mode}.${ext}`]),
    ),
  );
  ...
  const { error } = await supabase.rpc("update_gym_branding", {
    p_gym_id: workspace.gymId,
    ...(mode === "light"
      ? { p_clear_logo_light: true }
      : { p_clear_logo_dark: true }),
  });
