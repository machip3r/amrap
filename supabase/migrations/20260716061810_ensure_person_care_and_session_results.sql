-- Ensure care notes + session results exist (idempotent if 20260716060226 already applied).

create table if not exists public.person_gym_care (
...
create index if not exists person_gym_care_person_id_idx on public.person_gym_care (person_id);

comment on table public.person_gym_care is 'Coach-facing care notes per person at a gym (medical/injury adaptations)';

create table if not exists public.class_session_results (
...
create index if not exists class_session_results_gym_id_idx on public.class_session_results (gym_id);

create index if not exists class_session_results_person_id_idx on public.class_session_results (person_id);
...
drop policy if exists person_gym_care_select on public.person_gym_care;
...
drop policy if exists person_gym_care_manage on public.person_gym_care;
...
drop policy if exists class_session_results_select on public.class_session_results;
...
drop policy if exists class_session_results_manage on public.class_session_results;
