...
export function AmrapWatermark({ locale, label, className = "" }: Props) {
  return (
    <div
      className={`flex shrink-0 items-center justify-center border-t border-[var(--color-border)]/60 bg-[var(--color-bg)] py-2.5 ${className}`}
    >
      <Link
        href={`/${locale}`}
