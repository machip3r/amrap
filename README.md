...
**App:** SvelteKit at the **repo root** (`pnpm dev` → `http://localhost:5173`). Installable as a **PWA** (home-screen app shell; cold start → `/app` → dashboard or login, not marketing). Public marketing at **[amrap.space](https://www.amrap.space)** is SEO-ready (intent titles like “software para gimnasios”, www canonicals, sitemap, structured data, locale alternates). Agent rules: [`AGENTS.md`](AGENTS.md). Legacy Next.js archive (read-only): [`amrap-next/`](amrap-next/).
