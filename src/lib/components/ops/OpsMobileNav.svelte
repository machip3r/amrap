<script lang="ts">
	...
	const moreActive = $derived(
		split.more.some((item) => isOpsNavActive(pathname, opsNavHref(prefix, item.path)))
	);
...
{#snippet tabLink(item: OpsNavItemDef)}
	...
	<a {href} class={tabClass(active)} aria-current={active ? 'page' : undefined}>
		...
		<span class="max-w-full truncate text-xs font-semibold leading-tight">{label}</span>
	</a>
{/snippet}
...
>
	<div class="relative flex items-stretch justify-around px-1 pt-1">
		{#each leftTabs as item (item.id)}
			{@render tabLink(item)}
		{/each}

		{#if qrCode}
			<div class="relative flex min-w-[5rem] flex-1 items-start justify-center">
				<button
					type="button"
					onclick={() => (qrOpen = true)}
					class="absolute -top-6 flex h-16 w-16 items-center justify-center rounded-full bg-[var(--color-primary)] text-[var(--color-primary-on)] shadow-lg ring-4 ring-[var(--color-surface)] transition-transform active:scale-95"
				...
				<span
					class="mt-11 max-w-full truncate px-0.5 text-center text-xs font-semibold leading-tight text-[var(--color-muted)]"
				>
					{d.nav.myQr}
				</span>
			</div>
		...
		<button
			...
			aria-haspopup="dialog"
		>
			<MoreHorizontal class="h-7 w-7 shrink-0" aria-hidden="true" />
			<span class="max-w-full truncate text-xs font-semibold leading-tight">{d.nav.more}</span>
		</button>
	</div>
	<AmrapWatermark
		{locale}
		label={d.shell.poweredBy}
		class="border-[var(--color-border)] bg-[var(--color-surface)] pb-[max(0.5rem,env(safe-area-inset-bottom))]"
	/>
</nav>
...
<Dialog
	...
	bodyClass="px-2 py-2 pb-[max(0.75rem,env(safe-area-inset-bottom))]"
	autoFocus={false}
>
	<div class="space-y-0.5">
		{#if organizationName || gymName}
			<div class="mb-2 border-b border-[var(--color-border)] px-4 pb-3 pt-1">
				...
				{#if gymName && organizationName && gymName !== organizationName}
					<p class="truncate text-sm text-[var(--color-muted)]">{gymName}</p>
				{:else}
					<p class="truncate text-sm text-[var(--color-muted)]">{d.shell.gymAdmin}</p>
				{/if}
		...
		{/each}
		<LogoutButton {locale} class={moreLinkClass(false)} title={d.nav.logout}>
...
{#if qrCode}
	<Dialog
		...
		fullScreen
		bodyClass="flex flex-col items-center justify-center gap-6 px-6 py-8 pb-[max(2rem,env(safe-area-inset-bottom))] text-center"
		autoFocus={false}
	>
		{#if fullName}
		...
		/>
		<p class="max-w-sm break-all font-mono text-base text-[var(--color-muted)] sm:text-lg">
			{qrCode}
		</p>
	</Dialog>
{/if}
