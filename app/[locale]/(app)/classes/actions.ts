...
export async function createClassSchedule(
  ...
  if (
    ...
    parsed.data.days_of_week.length === 0
  ) {
...
export async function cancelClassSession(formData: FormData): Promise<void> {
  ...
  revalidatePath(`/${locale}/classes`, "page");
  revalidatePath(`/${locale}/classes/${idParsed.data}`, "page");
}
...
export async function cancelBookingStaff(formData: FormData): Promise<void> {
  ...
  revalidatePath(`/${locale}/classes`, "page");
  if (sessionId) revalidatePath(`/${locale}/classes/${sessionId}`, "page");
}
...
export async function duplicateClassToGym(
  ...
  const targetGymId = uuidSchema.safeParse(
    formString(formData, "target_gym_id"),
  );
