...
export async function createStaffOrTrainer(
  ...
  const roleLabel =
    parsed.data.role === "trainer"
      ? d.registerUser.roleTrainer
      : d.registerUser.roleStaff;
  ...
  return {
    ...
    emailWarning:
      invite.emailSent || invite.emailSkipped
        ? undefined
        : d.teamInvites.emailFailed,
  };
}
...
export async function removeTeamMemberAction(
  ...
  const locale = localeFromForm(formData);
  const workspace = await getWorkspace();
  const roleHint = formString(formData, "list_role"); // trainer | staff
  const back =
    roleHint === "trainer"
      ? `/${locale}/trainers`
      : `/${locale}/staff`;
  ...
  // Never allow removing your own gym role from the list.
