...
<div class="flex min-h-0 flex-1 flex-col overflow-hidden bg-[var(--color-bg)] pt-[var(--safe-top)]">
	...
	>
		...
		<div
			class="flex items-center justify-between gap-3 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] px-3 py-2.5"
		>
			<span class="text-sm font-medium text-[var(--color-text)]">{labels.type}</span>
			<div class="inline-flex rounded-lg bg-[var(--color-surface-hover)] p-0.5">
		...
		<div class="overflow-hidden rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)]">
			{#if draft.type === 'simple'}
				<ul class="divide-y divide-[var(--color-border)]">
					...
					<li
						class="flex min-h-12 items-center justify-between gap-3 bg-[var(--color-surface-hover)]/40 px-4 py-2.5"
					>
					...
					<li class="flex min-h-14 items-center justify-between gap-3 px-4 py-3">
						...
						{@render timePill(work?.seconds ?? 0, 'bg-rose-600', (s) => setPhaseSeconds('work', s))}
					</li>
					<li class="flex min-h-14 items-center justify-between gap-3 px-4 py-3">
						<span class="text-sm font-medium sm:text-base">{labels.phaseRest}</span>
						{@render timePill(rest?.seconds ?? 0, 'bg-green-600', (s) => setPhaseSeconds('rest', s))}
					</li>
					<li class="flex min-h-14 items-center justify-between gap-3 px-4 py-3">
			...
			{:else}
				<ul class="divide-y divide-[var(--color-border)]">
					...
					{#each draft.cycles ?? [] as cycle, idx (cycle.id)}
						<li class="bg-[var(--color-surface-hover)]/30">
								...
								<div class="flex items-center gap-2">
									...
									>
										...
										<input
											...
											class="min-h-10 w-16 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg)] px-2 text-center text-base font-semibold tabular-nums"
										/>
									</label>
									{#if (draft.cycles?.length ?? 0) > 1}
										<button
											type="button"
											onclick={() => removeCycle(cycle.id)}
											class="inline-flex h-10 w-10 items-center justify-center text-[var(--color-muted)] hover:text-[var(--color-danger)]"
					...
					<li>
						<button
							type="button"
							onclick={addCycle}
							class="flex min-h-12 w-full items-center gap-2 px-4 py-3 text-sm font-semibold text-green-700 hover:bg-green-500/10"
						>
		...
		<div
			class="sticky bottom-0 space-y-2 rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] px-4 py-3 shadow-sm"
		>
