...
export async function requestCreateGym(
  _prev: OrgActionState,
  formData: FormData,
): Promise<OrgActionState> {
  const locale = localeFromForm(formData);
  ...
  // Multi-gym create RPC lands with billing; keep the flow discoverable.
  return {
    success: true,
...
export async function scheduleGymDeletion(
  _prev: OrgActionState,
  formData: FormData,
): Promise<OrgActionState> {
  const locale = localeFromForm(formData);
  ...
  if (gym.id === workspace.gymId) {
    redirect(`/${locale}/organization?deleted=gym`);
  }

  return { success: true, message: d.organization.gymDeletionScheduled };
}

export async function cancelGymDeletion(
  _prev: OrgActionState,
  formData: FormData,
): Promise<OrgActionState> {
  const locale = localeFromForm(formData);
...
export async function scheduleOrganizationDeletion(
  ...
  // Soft-delete gyms in the same retention window.
  ...
  revalidatePath(`/${locale}/organization`, "page");
  revalidatePath("/", "layout");
  redirect(`/${locale}/login?orgDeleted=1`);
}
